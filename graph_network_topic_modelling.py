...
def get_top_n_topics(G, top_n, central_function = nx.centrality.degree_centrality):
...
def process_terms(terms, top_n = 10):
	G = build_full_graph(terms)
	Gconn = get_largest_connected_subgraph(G)
	topics = get_top_n_topics(Gconn, top_n)
...
def clean_topics(topics):
	cleaned_topics = []
	for t in topics:
		cleaned_topics.append(t.split(':')[-1])
	return cleaned_topics

def main():
	# terms = ['facebook', 'twitter', 'snapchat', 'foursquare', 'myspace']
	terms = ['uber', 'autonomous car', 'machine learning', 'artificial intelligence', 'neural networks']
	topics = process_terms(terms)
	print topics
